...
// Serial2 is the computer sending the midi stream
int LINMOT_BAUDRATE = 57600;
int USB_SERIAL_BAUDRATE = 57600;
int count = 0;

long posLa = 200;
long k = 440*posLa; // car f = k/L, à tension fixe
...
void loop(){
  if (Serial.available()) {
    ...
    if(ch=='M'){
      ...
      if(buffer[0] == channelHex){
        byte pitch = buffer[1];
        float f = pow(2., (pitch-69)/12.) * 440.;
        if(k/f > 20 && k/f < 530){
          linMotGoToPosMM(k/f);
        }
      }
